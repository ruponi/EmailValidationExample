...
@interface RSACell : UITableViewCell
@property (nonatomic, retain) UIView *separatorView;
@end

@implementation RSACell

@end

@interface AutocompleteInputView ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation AutocompleteInputView
...
- (id)initWithHeight:(CGFloat)height
{
    ...
    if (self) {
        // create the table view with suggestions
...
{
    if (show && self.hidden) {
        // to remove the first animation when the virtual keyboard appears
        // use the hidden property to hide the bar wihout animation
        self.hidden = NO;
    }
...
- (NSString *)stringForObjectAtIndex:(NSUInteger)index
{
    ...
    } else if ([object conformsToProtocol:@protocol(AutocompleteItem)]) {
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    [self show: YES withAnimation: NO];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    RSACell * cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (cell == nil) {
        cell = [[RSACell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    ...
    // customize the cell view if the data source supports it, otherwise the text is used
